...
/** Groups attendance rows by calendar date, returns sorted [{date, count}]. */
function groupByDate(rows: { time_in: string }[]): { date: string; count: number }[] {
  const map: Record<string, number> = {}
  for (const r of rows) {
    const d = new Date(r.time_in).toLocaleDateString('en-PH', {
...
async function fetchMonth(
  ...
  const { data, error } = await supabase
    ...
    .lte('time_in', lte)
    .order('time_in')
  if (error) throw new Error(`Supabase error (month ${month}): ${error.message}`)
  return data ?? []
}
...
async function fetchRange(
  ...
  let query = supabase.from('attendance_logs').select('time_in').order('time_in')
  ...
  if (error) throw new Error(`Supabase error (range): ${error.message}`)
  return data ?? []
}
...
export async function generateAllPdf(
  ...
  if (filter.type === 'month' && filter.months && filter.months.length > 0) {
    ...
    const rows = groupByDate(raw)
    const label =
      filter.type === 'day'    ? (filter.day ?? 'Selected Day') :
      filter.type === 'period' ? `${filter.from} – ${filter.to}` : 'Selected Period'
  ...
  const session = new PdfSession(
    { title: 'ALL ATTENDANCE REPORT', subtitle },
    totalPages,
  )
  ...
  for (const period of periods) {
    ...
    const startY = session.sectionTitle(
      `Daily Attendance — ${period.label}`,
      `Total visits: ${total.toLocaleString()}   ·   Days with records: ${period.rows.length}`,
    )
    ...
    session.doc.text(
      '■  Each bar represents one calendar day within the selected period.',
      14,
      bottomY + 5,
    )
  ...
  const tableBody = periods.flatMap(p =>
    p.rows.map(r => [p.label, r.date, r.count.toString()])
  )
