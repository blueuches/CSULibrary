...
export async function generateCollegePdf(
  ...
  const yearLevelLabels  = ['Year 1','Year 2','Year 3','Year 4','Year 5']
    .filter(l => byYearLevel[l])
  ...
  session.newPage()
  {
    ...
    const chart = await renderBarChart({
      ...
      datasets: [{ label: 'Visits', data: yearLevelLabels.map(l => byYearLevel[l]?.length ?? 0), color: '#2e7d32' }],
      yLabel:   'Number of Visits',
    })
    session.embedChart(chart, startY)
  }
  ...
  {
    const startY = session.sectionTitle(
      isSpecificProgram ? `${scope.program} — Year Level Hourly` : 'Year Levels — Hourly Attendance',
  ...
  session.newPage()
  {
    ...
    const startY = session.sectionTitle(
      isSpecificProgram ? `${scope.program} — Gender Distribution` : 'Programs — Gender Distribution',
  ...
  session.newPage()
  {
    ...
    const startY = session.sectionTitle(
      isSpecificProgram ? `${scope.program} — Year Level Gender` : 'Year Levels — Gender Distribution',
