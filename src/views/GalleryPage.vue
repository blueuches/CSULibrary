<template>
  <div class="w-full py-6 relative min-h-screen overflow-y-auto flex flex-col items-center">
    ...
    <div class="w-full sticky top-0 z-30 shadow-2xl">
      <div class="w-full bg-[#1b3a2f] text-yellow-400 text-center py-6 border-b border-yellow-400/20">
      ...
      <div class="w-full bg-[#1b3a2f]/90 backdrop-blur-md py-3 px-4 flex justify-start">
        <div class="relative w-full max-w-[300px] ml-2">
          ...
          />
          <svg class="absolute left-3 top-2.5 h-4 w-4 text-[#1b3a2f]/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-if="searchQuery" class="w-full max-w-5xl px-6 mt-8 mb-4 text-left">
      <h2 class="text-white text-xl font-bold bg-[#1b3a2f]/80 inline-block px-6 py-2 rounded-r-full border-l-4 border-yellow-400 backdrop-blur-sm">
      ...
      <transition-group name="fade" tag="div" class="w-full flex flex-col items-center">
        ...
        >
          <div v-if="searchQuery" class="w-full max-w-5xl mb-4">
             <span class="text-yellow-400 font-black text-sm uppercase tracking-widest pl-4">📍 {{ floor.name }}</span>
          </div>
          ...
          >
            ...
            <div class="flex flex-wrap justify-center gap-8">
              ...
              >
                <img :src="section.images[0]" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                
                <div class="absolute inset-0 bg-[#1b3a2f]/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center z-10 backdrop-blur-[2px]">
                   <div class="bg-yellow-400 text-[#1b3a2f] px-5 py-2 rounded-full font-black text-xs uppercase tracking-widest shadow-xl">
                    View Gallery
                  </div>
                </div>
                ...
                <div class="absolute bottom-6 left-8 right-6 z-20">
                  <div class="flex items-start gap-3">
                    <div class="w-1 h-6 bg-yellow-400 rounded-full shadow-[0_0_10px_rgba(250,204,21,0.5)]"></div>
                    <div>
                      <h4 class="text-white font-bold text-lg leading-tight uppercase">{{ section.title }}</h4>
                      <p v-if="section.note" class="text-yellow-400 text-[10px] font-black uppercase mt-1">
                        {{ section.note }}
                      </p>
                    </div>
                  </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/98 flex items-center justify-center z-50 p-6 backdrop-blur-md">
      <button @click="showModal = false" class="absolute top-10 right-10 text-white/50 hover:text-yellow-400 text-5xl">&times;</button>
      <button v-if="currentImages.length > 1" @click="prevImage" class="absolute left-6 text-yellow-400 text-6xl">&#10094;</button>
      <div class="flex flex-col items-center max-w-5xl w-full">
        <img :src="currentImages[currentIndex]" class="rounded-2xl shadow-2xl max-h-[75vh] object-contain" />
      </div>
      <button v-if="currentImages.length > 1" @click="nextImage" class="absolute right-6 text-yellow-400 text-6xl">&#10095;</button>
    </div>
  </div>
</template>
...
const floors = [
  { id: 'floor1', name: '1st Floor', wings: [{ name: 'Main Wing', sections: [{ title: 'Computer Lab', images: ['https://via.placeholder.com/800'] }, { title: 'Auditorium', images: ['https://via.placeholder.com/800'] }] }] },
  { id: 'floor2', name: '2nd Floor', wings: [
    { name: 'Right Wing', sections: [{ title: 'Discussion Rooms', note: 'Rooms 1-3', images: ['https://via.placeholder.com/800'] }, { title: 'Huddle Area', images: ['https://via.placeholder.com/800'] }] },
    { name: 'Left Wing', sections: [{ title: 'Filipiniana', images: ['https://via.placeholder.com/800'] }] }
  ]},
  { id: 'floor3', name: '3rd Floor', wings: [
    { name: 'Right Wing', sections: [{ title: 'Discussion Rooms', note: 'Rooms 1-4', images: ['https://via.placeholder.com/800'] }] },
    { name: 'Left Wing', sections: [{ title: 'Library Cafe', images: ['https://via.placeholder.com/800'] }] }
  ]}
]
...
const filteredFloors = computed(() => {
  ...
  return floors.map(floor => ({
    ...
    wings: floor.wings.map(wing => ({
      ...wing,
      sections: wing.sections.filter(s => 
